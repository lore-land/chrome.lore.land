...
export async function dispatchYankText(messageCallback) {
  try {
    ...
    parent.classList.add('__highlight', '__highlight__adding');

    const duration2 = 600;
    const duration1 = duration2 / 4;

    // Set custom CSS properties for animation durations
    parent.style.setProperty('--duration-1', `${duration1}ms`);
    parent.style.setProperty('--duration-2', `${duration2}ms`);

    // Handle removal of highlight classes in stages
    setTimeout(() => {
    ...
    console.error('Error in dispatchYankText:', error);
  }
}
